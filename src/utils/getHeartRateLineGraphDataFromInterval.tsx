...
export const getHeartRateLineGraphDataFromInterval = (
    ...
    const points: lineDataItem[] = interval.timeseries.heartRate.map(
        ([ts, v], index) => {
            ...
            return {
                ...
                label: index === 0 || index === interval.timeseries.heartRate.length - 1 ? dayjs(ts).utc().format("h:mm a") : undefined,
                labelTextStyle: styles.xAxisLabel,
                // This is meant to be rendered on the main graph but we use it as 
    ...
    points[minIdx].dataPointLabelComponent = () => dPointLabel(Math.floor(minPoint), false);
    points[minIdx].hideDataPoint = false;
    points[maxIdx].dataPointLabelComponent = () => dPointLabel(Math.floor(maxPoint), true);
...
const dPointLabel = (value: number, isHigh: boolean) => {
    return (
        <SleepText style={[isHigh ? styles.labelHigh : styles.labelLow, styles.label]}>{value}</SleepText>
    );
};
