...
export const PointerLabelComponent =
  (
    dataPoint: TimeseriesDataPoint<LineGraphBase>,
    type: 'degrees' | 'bpm',
    prefixes?: string[],
  ) =>
  (items: lineDataItem[]) => {
    ...
    if (type === 'degrees') {
      topLabel = strings.units.celcius(
        (item?.value || 0) + dataPoint.data.yAxisOffset,
      );
    } else if (type === 'bpm') {
      topLabel = strings.units.bpm(
        (item?.value || 0) + dataPoint.data.yAxisOffset,
      );
    }

    if (type === 'degrees' && item2) {
      midLabel = strings.units.celcius(
        (item2?.value || 0) + dataPoint.data.yAxisOffset,
      );
    }
