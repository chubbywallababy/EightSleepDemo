    ...
    return (
        <SleepView style={styles.cardContainer}>
            ...
            <View style={styles.progressLineContainer}>
                ...
                />
                <ProgressBar
                    toValue={(statValue - lineRange.min) / (lineRange.max - lineRange.min)}
                />
...
const GoalRange = ({minValue, maxValue, lineRangeMin, lineRangeMax}: GoalRangeProps) => {
    // Calculate the position of the goal range on the progress line
    const leftPosition = (minValue - lineRangeMin) / (lineRangeMax - lineRangeMin) * 100;
    const width = (maxValue - minValue) / (lineRangeMax - lineRangeMin) * 100;

    return (
        <View style={[styles.goalContainer, {left: `${leftPosition}%`, width: `${width}%`}]}>
            <DottedLine
                numDots={5}
                dir="vertical"
                containerStyle={styles.leftGoalPost}
            />
            <SleepText style={styles.goalText}>
                {strings.details.card.goal}
            </SleepText>
            <DottedLine
                numDots={5}
                dir="vertical"
