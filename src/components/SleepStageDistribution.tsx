...
import {SleepStageData, SleepStagePercentage, TimeseriesDataPoint} from '../utils/types';
 ...
 * @returns 
 */
export const SleepStageDistribution = ({dataPoint}: SleepStageDistributionProps) => {
    return (
        <View>
            ...
            <View style={[graphStyles.container, styles.content]}>
                <PieChart
                    ...
                    sectionAutoFocus
                />
                <LegendComponent percentages={dataPoint.data.percentages} total={dataPoint.data.totalTimeAsleep} />
            </View>
        </View>
    );
}
...
const getPieDataItems = (data: SleepStageData): pieDataItem[] => {
    return data.percentages.map((v) => ({value: round(v.percentage, 2), color: STAGE_COLOR_MAP[v.stage] || ''}))
}
...
const LegendComponent = ({percentages, total}: LegendComponentProps) => {
    ...
    return (
        <View style={styles.rowContainer}>
            <SleepText style={styles.totalTimeAsleep}>{`${strings.details.totalTimeAsleep} `}{strings.units.getHoursAndMinutes(hours, minutes)}</SleepText>
            {percentages.map((p) => (
...
const PercentageItem = ({data}: PercentageItemProps) => {
    return (
        <View style={styles.percentage}>
            <View style={[styles.circle, {backgroundColor: STAGE_COLOR_MAP[data.stage]}]} />
            <SleepText>{data.stage}</SleepText>
            <SleepText>{strings.units.asPercent((round(data.percentage * 100, 2)))}</SleepText>
        </View>
    )
}
